...
namespace Logging
{
    public class HYLDDebug
    {
        ...
        {
            Logger.Warn(0, prefix + format.ToString(), args);
        }
        ...
        {
            Logger.Err(0, prefix + format.ToString(), args);
        }
    ...
    public static class Logger
    {
        ...
        public static void DefaultServerLogHandler(object sernder, LogEventArgs logArgs)
        {
            ...
            if (logArgs.LogSeverity == LogSeverity.Error|| logArgs.LogSeverity == LogSeverity.Exception)
            {
                UnityEngine.Debug.LogError(logArgs.Message);
            }
            else if (logArgs.LogSeverity == LogSeverity.Warn)
            {
                UnityEngine.Debug.LogWarning(logArgs.Message);
            }
            else UnityEngine.Debug.Log(logArgs.Message);

            return;
            if (_logBuffer.Length != 0)
            {
                UnityEngine.Debug.Log(_logBuffer.ToString());
                _logBuffer.Length = 0;
                _logBuffer.Clear();
            }
        }
