	...
	public class BTParallel : BTNode {
		...
		protected override bool DoEvaluate () {
			foreach (BTNode child in children) {
				//Logging.HYLDDebug.LogError($"BTParelle.child={child}");
				if (!child.Evaluate()) {
					return false;	
				}
			}
		...
		public override BTResult Tick () {
			...
			{   // For debug
				//Logging.HYLDDebug.Log("Tick开始 " + this.name + " [" + this.GetType().ToString() + "]"+"("+_func+")"+"children::"+children[0]+children[1]);
			...
			for (int i=0; i<children.Count; i++) {
				...
				if (_func == ParallelFunction.And) {
					...
					//Logging.HYLDDebug.LogError($"Tick过程{GetType().ToString()}  result[{i}]{children[i]}={_results[i]}");
					if (_results[i] == BTResult.Running) {
						_results[i] = children[i].Tick();	
					}
					if (_results[i] != BTResult.Running) {
