	...
	public class BTParallelFlexible : BTNode {
		...
		public override BTResult Tick () {
			...
			for (int i=0; i<_children.Count; i++) {
				...
				//Logging.HYLDDebug.LogError($"_activeList{i}:{_children[i]}={_activeList[i]}");
				if (active) {
					//Logging.HYLDDebug.LogError($"Tick BTParallelFlexible children{i}={_children[i]} {_children.Count}");
